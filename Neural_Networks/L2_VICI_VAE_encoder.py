...
class VariationalAutoencoder(object):
    ...
    def _calc_z_mean_and_sigma(self,x):
        with tf.name_scope("L2_VICI_VAE_encoder"):
            hidden1_pre = tf.add(tf.matmul(x, self.weights['L2_VICI_VAE_encoder']['W3_to_hidden']), self.weights['L2_VICI_VAE_encoder']['b3_to_hidden'])
...
#            hidden2_post = hidden1_post
            ...
            z_mean = tf.add(tf.matmul(hidden1_post, self.weights['L2_VICI_VAE_encoder']['W4_to_mu']), self.weights['L2_VICI_VAE_encoder']['b4_to_mu'])
#            z_mean = self.nonlinearity2(z_mean)
#            z_mean = tf.exp(z_mean)
            z_log_sigma_sq = tf.add(tf.matmul(hidden1_post, self.weights['L2_VICI_VAE_encoder']['W5_to_log_sigma']), self.weights['L2_VICI_VAE_encoder']['b5_to_log_sigma'])
#            z_log_sigma_sq = self.nonlinearity(z_log_sigma_sq+10)-10
    ...
    def _create_weights(self):
        ...
        with tf.variable_scope("L2_VICI_VAE_ENC"):
            # Encoder
            all_weights['L2_VICI_VAE_encoder'] = collections.OrderedDict()
            hidden_number_encoder = self.n_weights
            all_weights['L2_VICI_VAE_encoder']['W3_to_hidden'] = tf.Variable(vae_utils.xavier_init(self.n_input, hidden_number_encoder), dtype=tf.float32)
            tf.summary.histogram("W3_to_hidden", all_weights['L2_VICI_VAE_encoder']['W3_to_hidden'])
...
#            tf.summary.histogram("W3c_hth", all_weights['VAE_encoder']['W3c_hth'])
    ...
    #        tf.summary.histogram("W3d_hth", all_weights['encoder']['W3d_hth'])
            ...
            all_weights['L2_VICI_VAE_encoder']['W4_to_mu'] = tf.Variable(vae_utils.xavier_init(hidden_number_encoder, self.n_hidden),dtype=tf.float32)
            tf.summary.histogram("W4_to_mu", all_weights['L2_VICI_VAE_encoder']['W4_to_mu'])
    
            all_weights['L2_VICI_VAE_encoder']['W5_to_log_sigma'] = tf.Variable(vae_utils.xavier_init(hidden_number_encoder, self.n_hidden), dtype=tf.float32)
            tf.summary.histogram("W5_to_log_sigma", all_weights['L2_VICI_VAE_encoder']['W5_to_log_sigma'])
    
            all_weights['L2_VICI_VAE_encoder']['b3_to_hidden'] = tf.Variable(tf.zeros([hidden_number_encoder], dtype=tf.float32) * self.bias_start)
            all_weights['L2_VICI_VAE_encoder']['b3_hth'] = tf.Variable(tf.zeros([hidden_number_encoder], dtype=tf.float32) * self.bias_start)
            all_weights['L2_VICI_VAE_encoder']['b3b_hth'] = tf.Variable(tf.zeros([hidden_number_encoder], dtype=tf.float32) * self.bias_start)
            all_weights['L2_VICI_VAE_encoder']['b3c_hth'] = tf.Variable(tf.zeros([hidden_number_encoder], dtype=tf.float32) * self.bias_start)
            all_weights['L2_VICI_VAE_encoder']['b3d_hth'] = tf.Variable(tf.zeros([hidden_number_encoder], dtype=tf.float32) * self.bias_start)
            all_weights['L2_VICI_VAE_encoder']['b4_to_mu'] = tf.Variable(tf.zeros([self.n_hidden], dtype=tf.float32) * self.bias_start, dtype=tf.float32)
            all_weights['L2_VICI_VAE_encoder']['b5_to_log_sigma'] = tf.Variable(tf.zeros([self.n_hidden], dtype=tf.float32) * self.bias_start, dtype=tf.float32)
