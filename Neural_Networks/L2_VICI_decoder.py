...
class VariationalAutoencoder(object):
    ...
    def calc_reconstruction(self, z):
        with tf.name_scope("L2_VICI_decoder"):
            if self.middle == "bernoulli":
                ...
                hidden1_pre = tf.add(tf.matmul(z,self.weights['L2_VICI_decoder']['W3_to_hiddenG']), self.weights['L2_VICI_decoder']['b3_to_hiddenG'])
                hidden1_post = self.nonlinearity(hidden1_pre)
...
#                hidden1b_post = hidden1_post
                ...
                mu = tf.add(tf.matmul(hidden1_post, self.weights['L2_VICI_decoder']['W4_to_muG']), self.weights['L2_VICI_decoder']['b4_to_muG'])
                mu = tf.sigmoid(mu)  # see paper
#                mu = tf.sigmoid(mu+0.5)-0.5  # see paper
                log_sigma_sq = tf.add(tf.matmul(hidden1_post, self.weights['L2_VICI_decoder']['W5_to_log_sigmaG']), self.weights['L2_VICI_decoder']['b5_to_log_sigmaG'])
