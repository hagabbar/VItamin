...
class VariationalAutoencoder(object):
    ...
    def _calc_z_mean_and_sigma(self,x):
        with tf.name_scope("L2_VICI_encoder"):
            hidden1_pre = tf.add(tf.matmul(x, self.weights['L2_VICI_encoder']['W3_to_hidden']), self.weights['L2_VICI_encoder']['b3_to_hidden'])
...
#            hidden2_post = hidden1_post
            ...
            z_mean = tf.add(tf.matmul(hidden1_post, self.weights['L2_VICI_encoder']['W4_to_mu']), self.weights['L2_VICI_encoder']['b4_to_mu'])
#            z_mean = self.nonlinearity2(z_mean)
#            z_mean = tf.exp(z_mean)
            z_log_sigma_sq = tf.add(tf.matmul(hidden1_post, self.weights['L2_VICI_encoder']['W5_to_log_sigma']), self.weights['L2_VICI_encoder']['b5_to_log_sigma'])
