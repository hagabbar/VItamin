...
class VariationalAutoencoder(object):
    ...
    def calc_reconstruction(self, z):
        with tf.name_scope("ELBO_decoder"):
            if self.middle == "bernoulli":
                ...
                hidden1_pre = tf.add(tf.matmul(z,self.weights['ELBO_decoder']['W3_to_hiddenG']), self.weights['ELBO_decoder']['b3_to_hiddenG'])
                hidden1_post = self.nonlinearity(hidden1_pre)
...
#                hidden1e_post = self.nonlinearity(hidden1e_pre)
                ...
                hidden1b_pre = tf.add(tf.matmul(hidden1_post, self.weights['ELBO_decoder']['W3b_to_hiddenG']), self.weights['ELBO_decoder']['b3b_to_hiddenG'])
                hidden1b_post = self.nonlinearity(hidden1b_pre)
#                hidden1b_post = hidden1_post

                mu = tf.add(tf.matmul(hidden1b_post, self.weights['ELBO_decoder']['W4_to_muG']), self.weights['ELBO_decoder']['b4_to_muG'])
#                mu = 0.3*(tf.sigmoid(mu)-0.5)
#                mu = tf.sigmoid(mu)  # see paper
#                mu = tf.sigmoid(mu+0.5)-0.5  # see paper
                log_sigma_sq = tf.add(tf.matmul(hidden1b_post, self.weights['ELBO_decoder']['W5_to_log_sigmaG']), self.weights['ELBO_decoder']['b5_to_log_sigmaG'])
...
#                log_sigma_sq = self.nonlinearity(log_sigma_sq+6)-6
    ...
    def _create_weights(self):
        ...
        # Decoder
        with tf.variable_scope("ELBO_DEC"):
            all_weights['ELBO_decoder'] = collections.OrderedDict()
            if self.middle == "gaussian":
                hidden_number_decoder = self.n_weights
                all_weights['ELBO_decoder']['W3_to_hiddenG'] = tf.Variable(vae_utils.xavier_init(self.n_hidden, hidden_number_decoder), dtype=tf.float32)
                all_weights['ELBO_decoder']['b3_to_hiddenG'] = tf.Variable(tf.zeros([hidden_number_decoder], dtype=tf.float32)  * self.bias_start)
    ...
    #            all_weights['decoder']['b3_to_hiddenGS'] = tf.Variable(tf.zeros([hidden_number_decoder], dtype=tf.float32)  * self.bias_start)
                ...
                all_weights['ELBO_decoder']['W3b_to_hiddenG'] = tf.Variable(vae_utils.xavier_init(hidden_number_decoder, hidden_number_decoder), dtype=tf.float32)
                all_weights['ELBO_decoder']['b3b_to_hiddenG'] = tf.Variable(tf.zeros([hidden_number_decoder], dtype=tf.float32)  * self.bias_start)
    #            
    ...
    #            all_weights['decoder']['b3e_to_hiddenG'] = tf.Variable(tf.zeros([hidden_number_decoder], dtype=tf.float32)  * self.bias_start)
                ...
                all_weights['ELBO_decoder']['W4_to_muG'] = tf.Variable(vae_utils.xavier_init(hidden_number_decoder, self.n_input), dtype=tf.float32)
                all_weights['ELBO_decoder']['b4_to_muG'] = tf.Variable(tf.zeros([self.n_input])  * self.bias_start, dtype=tf.float32)
                all_weights['ELBO_decoder']['W5_to_log_sigmaG'] = tf.Variable(vae_utils.xavier_init(hidden_number_decoder, self.n_input), dtype=tf.float32)
                all_weights['ELBO_decoder']['b5_to_log_sigmaG'] = tf.Variable(tf.zeros([self.n_input])  * self.bias_start, dtype=tf.float32)
