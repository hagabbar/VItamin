...
def train(params, x_data, y_data_h, load_dir, save_dir):
    ...
    with graph.as_default():
        ...
        autoencoder = L2_VICI_decoder.VariationalAutoencoder("L2_VICI_decoder", xsh[1], z_dimension+ysh1, n_weights)
        autoencoder_ENC = L2_VICI_encoder.VariationalAutoencoder("L2_VICI_encoder", ysh1, z_dimension, n_weights)
        autoencoder_VAE = L2_VICI_VAE_encoder.VariationalAutoencoder("L2_VICI_VAE_encoder", xsh[1]+ysh1, z_dimension, n_weights)
        ...
        # GET r(z|y)
        y_ph_pre = tf.placeholder(dtype=tf.float32, shape=[None, ysh1], name="y_ph")
    ...
    for i in range(params['num_iterations']):
        ...
        ynn = session.run(qx_samp_pre,feed_dict={y_ph_pre:yn,bs_ph:bs})
        session.run(minimize, feed_dict={bs_ph:bs, x_ph:x_data[next_indices, :],  y_ph:ynn, lam_ph:lam}) # minimising cost function
        ...
        if i % params['report_interval'] == 0:
                ...
                cost_value_vae, KL_VAE = session.run([COST_VAE, KL_vae], feed_dict={bs_ph:test_n, x_ph:x_data[0:test_n,:], y_ph:ynnt, lam_ph:lam})
                KL_PLOT[ni] = KL_VAE
...
def run(params, y_data_test, siz_x_data, load_dir_pre, load_dir):
    ...
    with graph.as_default():
        ...
        # LOAD VICI NEURAL NETWORKS
        autoencoder = L2_VICI_decoder.VariationalAutoencoder("L2_VICI_decoder", xsh1, z_dimension+ysh1, n_weights)
        autoencoder_ENC = L2_VICI_encoder.VariationalAutoencoder("L2_VICI_encoder", ysh1, z_dimension, n_weights)
        autoencoder_VAE = L2_VICI_VAE_encoder.VariationalAutoencoder("L2_VICI_VAE_encoder", xsh1+ysh1, z_dimension, n_weights)
    ...
    ns = 20 # number of samples to use to estimate per-pixel marginal
    ...
    for i in range(ns):
        ...
        rec_x_m = session.run(x_mean,feed_dict={y_ph:ynn})
        rec_x_mx = session.run(qx_samp,feed_dict={y_ph:ynn})
        rec_x_s = session.run(x_mean,feed_dict={y_ph:ynn})
        XM[:,:,i] = rec_x_m
        XSX[:,:,i] = rec_x_mx
        XSA[:,:,i] = rec_x_s
