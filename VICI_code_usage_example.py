...
iterations = 3000000  # total number of iterations on inverse
...
def get_params():
    params = dict(
        ...
        n_weights = 3000, # number of dimensions of the intermediate layers of encoders and decoders in the inference model (inverse reconstruction)
        ...
        z_dimensions_fw = 50, # latent space dimensionality of forward model
        ...
        dt = T/ndata,        # sampling time (Sec)
        fnyq = 0.5/dt,   # Nyquist frequency (Hz),
        train_set_dir='training_sets/tset_tot-%d_split-%d' % (tot_dataset_size,tset_split), #location of training set
        test_set_dir='test_sets/tset_tot-%d_freq-%d' % (r*r,ndata), #location of training set
        add_noise_real=True, # whether or not to add extra noise realizations in training set
        n_noise=1,          # number of noise realizations
...
forward_loss1, forward_loss2 = VICI_forward_model.train(params, x_data_train_h, y_data_train_h, y_data_train_lh, "forward_model_dir_%s/forward_model.ckpt" % params['run_label'], plotter) # This trains the forward model and saves the weights in forward_model_dir/forward_model.ckpt
...
for i in range(int(iterations/(params['num_iterations']-1))):
    ...
    if (i*int(params['num_iterations']-1)) % 50000 == 0:
        # The trained inverse model weights can then be used to infer a probability density of solutions given new measurements
        xm, xsx, XS, pmax = VICI_inverse_model.run(params, y_data_test_h, np.shape(x_data_train)[1], "inverse_model_dir_%s/inverse_model.ckpt" % params['run_label']) # This runs the trained model using the weights stored in inverse_model_dir/inverse_model.ckpt
"""
